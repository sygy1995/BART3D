...
def main(viewregion,normalization,chrom):
    ...
    outdir = 'f3_UDHS_bart_input'
    os.makedirs(outdir,exist_ok=True) 
    
    infiles = glob.glob(indir+'/*.csv')#;print(infiles);exit()
    for infile in infiles:
        #infile = 'f1_UDHS_interaction_change/Jurkat_over_A6010_raw_res5000_view200000.csv'
        basename = os.path.basename(infile).split('.csv')[0]
        with open(infile) as inf:
            df = pd.read_csv(inf,sep='\t',index_col=0)
        #df = df[['stats']]
        target = df[(df['treat_mean']==0)|(df['ctrl_mean']==0)].index
        df.set_value(target,'stats',0)
        df = df.fillna(0)
        #df = df.sort_values(by=['stats'])
        #df.to_csv('test.csv')
        #print(infile,df);exit()
        df = df[['stats']]
